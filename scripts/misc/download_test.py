...
logging.basicConfig(filename='C:/Users/amcclary/Documents/GitHub/Demographics/MyLog.log',level=logging.DEBUG)
...
def get_variable_data_minimal(year, dataset, geometry_return, variable, variablename, census_api_key, census_geom_year, tahoe_geometry, variable_category):
    #Returns all data for a given dataset for Washoe, El Dorado, Carson City, Douglas, Placer Counties
    #Need to make five seperate api calls because of the geometry structure
    county_states ={
        '06': ['017','061'],
...
def census_download_wrapper_minimal (variable_file):
    dtypes = {
    'Variable' : str,
    'Code': str,
    'Category': str,
    ...
    for index, row in variables.iterrows():
        print(index)
        try:
            df = get_variable_data_minimal(row['Year'], row['Datasource Name'],row['GeometryLevel'],row['CodeNumber'],row['Variable'], census_api_key, row['census_geom_year'], tahoe_geometry, row['Category'])
...
def load_variable_multiple_year(year_range, dataset, geometry_return, variable, variablename, census_api_key, tahoe_geometry, variable_category):
    ...
    return df_return
def census_download_wrapper_multiple_years (variable_file, year_range):
    dtypes = {
    'Variable' : str,
    'Code': str,
    'Category': str,
    ...
    for index, row in variables.iterrows():
        print(index)
        try:
            df = load_variable_multiple_year(year_range, row['Datasource Name'],row['GeometryLevel'],row['CodeNumber'],row['Variable'], census_api_key,  tahoe_geometry, row['Category'])
    ...
    return df_values
try:
    all_2022 = census_download_wrapper_minimal('Census_Variable_Lists/acs_2022_bg_4.csv')
    all_2022.to_excel('acs_2022_bg_4.xlsx')
# catch system errors
except Exception:
    e = sys.exc_info()[1]
    print(e.args[0])
...
# # dashboard_2022.to_excel('climate_dashboard_2022.xlsx')
# #year_range = [ '2011','2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019','2020','2021','2022']
# year_range = [ '2011','2012', '2013']
# heating_info_1 = census_download_wrapper_multiple_years('Census_Variable_Lists\climate_dashboard_heating_method_2022.csv', year_range)
# heating_info_1.to_excel('heating_info_3.xlsx')